...
class SpikeGui:
    ...
    def run(self) -> None:
        try:
            ...
            self.root.attributes('-alpha', 1.0)
            # No full update() here: on macOS (Cocoa Tk) `root.update()` blocks
            # until events drain and can hang forever before the window ever
            # reaches the screen. update_idletasks() lays everything out, and
            # mainloop() below pumps the real events that draw/map the window.
            self.root.update_idletasks()
            self.root.after(2000, lambda: self.root.attributes('-topmost', False))

            # ---- fullscreen (F11 / Escape) -------------------------------
            # macOS Cocoa Tk honours `-fullscreen` natively; on Apple Tk 8.5/8.6
            # both values are accepted and trigger the Aqua fullscreen. F11
            # toggles, Escape always brings the window back to normal sized.
            self._saved_geom = self.root.geometry()

            def _set_fs(on: bool) -> None:
                if on:
                    self._saved_geom = self.root.geometry()
            ...
            self.root.bind_all("<F11>", lambda _e: _set_fs(
                not bool(self.root.attributes('-fullscreen'))))
            self.root.bind_all("<Escape>", lambda _e: _set_fs(False))
